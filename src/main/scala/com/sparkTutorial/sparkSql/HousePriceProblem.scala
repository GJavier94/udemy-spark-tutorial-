...
object HousePriceProblem {
  ...
  val AVERAGE_PRICE_SQ_FT = "average_price_sq_ft"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val prices_per_location = responses.groupBy(responses(LOCATION) ).
      agg(avg(PRICE_SQ_FT).alias(AVERAGE_PRICE_SQ_FT))

    prices_per_location.show()

    val prices_per_location_ordered = prices_per_location.orderBy(AVERAGE_PRICE_SQ_FT)


    prices_per_location_ordered.show(10)
